...
def __init__(self, say, keyword): self.say = say
self.keyword = keyword def run(self, voice_command):
...
response = "ok," + move else:
self.say("ok, you played," + move)
while ("move" not in response):
response = p.stdout.readline().decode("utf-8") logging.info("Chess log: %s", response)
logging.info("Chess: %s", response) self.say(response)
